...
# Function for HTML Bill Template
def generate_html_bill_template(bill_no, current_date, cust_name, cust_phone, items_tuples, total_bill):
    table_rows_html = ""
    for idx, item in enumerate(items_tuples, start=1):
        table_rows_html += f"""
        <tr>
...
# 🌟 मुख्य २ टॅब्स (१. नवीन बिल बनवा | २. 🔍 मागील बिल शोधा व री-प्रिंट करा)
main_tab1, main_tab2 = st.tabs(["📝 नवीन बिल तयार करा", "🔍 मागील बिल शोधा व री-प्रिंट करा"])

# ------------------------------------------
# टॅब १: नवीन बिल तयार करा
# ------------------------------------------
with main_tab1:
    ...
    if st.session_state.items_list:
        ...
        if st.button("🖨️ Generate Full Bill & WhatsApp Preview", use_container_width=True):
            if not cust_name.strip():
                ...
                # HTML बिल जनरेट
                html_bill = generate_html_bill_template(bill_no, current_date, cust_name, cust_phone, st.session_state.items_list, total_bill)
                
                # स्क्रीनवर व्हॉट्सॲप मेसेज प्रिव्ह्यू दाखवणे
                st.markdown("### 📸 व्हॉट्सॲप प्रिव्ह्यू (स्क्रीनशॉट काढा):")
                wa_bubble = f"""
                <b>🧾 BALAJI CYBER POINT 🧾</b><br>
                <i>डिजिटल शासकीय सेवा केंद्र, माणगाव</i><br>
                ───────────────────────<br>
                प्रिय <b>{cust_name}</b>,<br>
                आपले बिल यशस्वीरित्या तयार झाले आहे:<br>
                <br>
                📄 <b>बिल नंबर:</b> `{bill_no}`<br>
                📅 <b>तारीख/वेळ:</b> {wa_date}<br>
                🛠️ <b>कामाचा तपशील:</b> {all_services_text}<br>
                💰 <b>एकूण रक्कम:</b> ₹ <b>{total_bill:.2f}/-</b><br>
                ───────────────────────<br>
                👍 <b>STATUS: PAID</b><br>
...
# ------------------------------------------
# 🔍 टॅब २: मागील बिल शोधा आणि री-प्रिंट करा
# ------------------------------------------
with main_tab2:
    st.markdown("<h3 style='color: #0056b3; text-align: center;'>🔍 गुगल शीटमधून मागील बिल शोधा</h3>", unsafe_allow_html=True)
    st.write("गुगल शीटमधील सर्व जुन्या बिलांची यादी खालीलप्रमाणे आहे. तुम्ही नाव, मोबाईल नंबर किंवा बिल नंबर टाईप करून ते शोधू शकता:")
    
    search_query = st.text_input("🔎 शोधा (ग्राहक नाव / मोबाईल / बिल नंबर):", key="search_bill_input")
    
    if st.button("🔄 डेटा रिफ्रेश करा / सर्व बिले लोड करा", use_container_width=True):
        st.rerun()
        
    records = fetch_all_saved_bills()
    
    if not records or len(records) <= 1:
        st.info("ℹ️ सध्या गुगल शीटमध्ये कोणतेही जुने बिल सापडले नाही.")
    else:
        headers = records[0] # ["तारीख/वेळ", "बिल नंबर", "ग्राहक नाव", "मोबाईल नंबर", "कामाचा प्रकार", "एकूण रक्कम"]
        rows = records[1:]
        ...
        if search_query.strip():
            ...
            filtered_rows = rows[-20:] # शेवटची २० बिले दाखवणे

        st.write(f"📊 **एकूण सापडलेली बिले: {len(filtered_rows)}**")
        
        for idx, row in enumerate(reversed(filtered_rows), start=1):
            if len(row) >= 6:
                b_date, b_no, b_cust, b_phone, b_work, b_total = row[0], row[1], row[2], row[3], row[4], row[5]
                
                with st.expander(f"🧾 {b_no} | {b_cust} | ₹ {b_total}/- ({b_date})"):
                    st.write(f"👤 **ग्राहक नाव:** {b_cust}")
                    st.write(f"📞 **मोबाईल नंबर:** {b_phone if b_phone else '-'}")
                    st.write(f"🛠️ **कामाचा तपशील:** {b_work}")
                    st.write(f"💰 **एकूण रक्कम:** ₹ {b_total}/-")
                    st.write(f"📅 **तारीख/वेळ:** {b_date}")
                    
                    try:
                        tot_val = float(b_total)
                    except:
                        tot_val = 0.0
                    
                    # पुनर्जन्म बिल तयार करणे
                    past_items = [(b_work, tot_val)]
                    past_html = generate_html_bill_template(b_no, b_date, b_cust, b_phone, past_items, tot_val)
                    
                    st.download_button(
                        label=f"📥 {b_no} बिलाची A5 HTML फाईल पुन्हा डाऊनलोड करा",
                        data=past_html,
                        file_name=f"Full_Bill_{b_no}.html",
                        mime="text/html",
                        key=f"dl_past_{b_no}_{idx}"
                    )
