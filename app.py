...
# ==========================================
# 🔐 सुरक्षा तपासणी (लॉगिन लॉक)
# ==========================================
if "user_role" not in st.session_state or st.session_state.user_role is None:
    st.warning("⚠️ कृपया आधी मुख्य पेजवर (app) जाऊन लॉगिन करा!")
    st.stop()

# ==========================================
# गुगल SHEET जोडणी (User Tracking सह)
# ==========================================
def get_next_bill_number_and_save(row_data_without_no):
    try:
        ...
        try:
            ...
            # नवीन शीट तयार करताना 'बनवणारा युझर' रकाना जोडला
            sheet.append_row(["तारीख/वेळ", "बिल नंबर", "ग्राहक नाव", "मोबाईल नंबर", "कामाचा प्रकार", "एकूण रक्कम", "बनवणारा युझर"])
        ...
        # 🎯 चालू लॉगिन असलेल्या युझरचे नाव शीटमध्ये पाठवणे
        current_user = st.session_state.user_role
        
        full_row_data = [
            row_data_without_no[0], 
            final_bill_no, 
            row_data_without_no[1], 
            row_data_without_no[2], 
            row_data_without_no[3], 
            row_data_without_no[4],
            current_user  # ७ व्या रकान्यात नाव सेव्ह होईल
        ]
...
if st.session_state.items_list:
    ...
    if st.button("🖨️ Generate Full Bill & WhatsApp Preview", use_container_width=True):
        if not cust_name.strip():
            ...
            # मूळ कडक A5 डबल-पेज HTML बिल टेम्पलेट
...
"""
            ...
            st.markdown("### 📸 व्हॉट्सॲप प्रिव्ह्यू:")
            wa_bubble = f"""
            ...
            if sheet_success:
                st.success(f"✅ हिशोब गुगल शीटमध्ये `{st.session_state.user_role}` च्या नावावर सुरक्षित सेव्ह झाला आहे!")
